...
class TestListFilters:
    ...
    def test_list_filters_rich_format(self):
        """Should format filters for rich output."""
        stdout, stderr, code = run_cli_raw("jira", "filters", "--format", "rich")
        assert code == 0
...
class TestGetFilter:
    ...
    def test_get_filter_rich_format(self):
        """Should format filter for rich output."""
        result = run_cli("jira", "filters", expect_success=False)
        data = get_data(result)
        if isinstance(data, list) and len(data) > 0:
            filter_id = str(data[0].get("id", ""))
            if filter_id:
                stdout, stderr, code = run_cli_raw("jira", "filter", filter_id, "--format", "rich")
                assert code == 0
