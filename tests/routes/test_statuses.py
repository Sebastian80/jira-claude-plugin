...
class TestListStatuses:
    ...
    def test_list_statuses_rich_format(self):
        """Should format statuses for rich output."""
        stdout, stderr, code = run_cli_raw("jira", "statuses", "--format", "rich")
        assert code == 0
...
class TestGetStatus:
    ...
    def test_get_status_rich_format(self):
        ...
        if len(data) > 0:
            status_name = data[0].get("name", "")
            if status_name:
                stdout, stderr, code = run_cli_raw("jira", "status", status_name, "--format", "rich")
                assert code == 0
