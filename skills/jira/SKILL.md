...
--format rich      # Terminal colors and panels (opens in tmux window if available)
--format markdown  # Markdown tables
```

**Tmux integration:** When using `--format rich` inside tmux, output opens in a new window with clickable links. Requires tmux 3.4+ with hyperlinks enabled.
